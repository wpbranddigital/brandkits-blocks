...
function Edit( { attributes, setAttributes } ) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Plan Details', 'brandkit-blocks' ) } initialOpen={ true }>
					<TextControl
						label={ __( 'Plan Title', 'brandkit-blocks' ) }
						value={ planTitle }
						onChange={ ( val ) => setAttributes( { planTitle: val } ) }
					/>
					<div style={ { display: 'flex', gap: '8px' } }>
						<TextControl
							label={ __( 'Currency', 'brandkit-blocks' ) }
							value={ currencySymbol }
							onChange={ ( val ) => setAttributes( { currencySymbol: val } ) }
							style={ { width: '60px' } }
						/>
						<TextControl
							label={ __( 'Price', 'brandkit-blocks' ) }
							value={ price }
							onChange={ ( val ) => setAttributes( { price: val } ) }
							style={ { flexGrow: 1 } }
						/>
						<TextControl
							label={ __( 'Period', 'brandkit-blocks' ) }
							value={ billingPeriod }
							onChange={ ( val ) => setAttributes( { billingPeriod: val } ) }
						/>
					</div>
					<TextareaControl
						label={ __( 'Features (one per line)', 'brandkit-blocks' ) }
				...
				<PanelBody title={ __( 'Popular Badge & CTA', 'brandkit-blocks' ) } initialOpen={ false }>
					<ToggleControl
						label={ __( 'Mark as Popular', 'brandkit-blocks' ) }
						checked={ isPopular }
						onChange={ ( val ) => setAttributes( { isPopular: val } ) }
					/>
					{ isPopular && (
						<TextControl
							label={ __( 'Badge Text', 'brandkit-blocks' ) }
							value={ badgeText }
					...
					/>
					<ToggleControl
						label={ __( 'Open link in new tab', 'brandkit-blocks' ) }
						checked={ buttonTarget }
						onChange={ ( val ) => setAttributes( { buttonTarget: val } ) }
					/>
				</PanelBody>

				<PanelBody title={ __( 'Card Styling', 'brandkit-blocks' ) } initialOpen={ false }>
					<RangeControl
					...
					colorSettings={ [
						{
							...
							label: __( 'Primary/Brand Color', 'brandkit-blocks' ),
						},
						{
							value: cardBgColor,
							onChange: ( val ) => setAttributes( { cardBgColor: val } ),
							label: __( 'Card Background', 'brandkit-blocks' ),
						},
						{
							value: textColor,
							onChange: ( val ) => setAttributes( { textColor: val } ),
							label: __( 'Text Color', 'brandkit-blocks' ),
						},
						{
							value: btnTextColor,
